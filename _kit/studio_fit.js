...
// category), present + menu, across phone widths. Re-measures after a settle so
// a transient mid-reflow reading never false-flags.
...
(async()=>{
 ...
 async function fit(label,vw){
  const meas=async()=>{const d=JSON.parse(await ev(EXPR));return {over:Math.max(d.body,d.doc)-vw,detail:d};};
  let m=await meas();
  if(m.over>2){await sleep(500);m=await meas();}   // re-measure after settle: kill transient reflow false-flags
  return {label,vw,over:m.over,detail:m.detail};
 }
 ...
 for(const vw of [360,390]){
  ...
  await send('Page.navigate',{url:SRV+'/?cb='+Date.now()});await sleep(2800);
  let f=await fit('home',vw);if(f.over>2)flags.push(f);console.log(`w${vw} home        over=${f.over}  (${f.detail.el})`);
  await ev(`__go("gallery");S.format="listing";openGallery();1`);await sleep(800);
  f=await fit('gallery',vw);if(f.over>2)flags.push(f);console.log(`w${vw} gallery     over=${f.over}  (${f.detail.el})`);
  for(const cat of cats){
   await ev(`S.f={};S.format=${JSON.stringify(cat)};S.template="folio";openEditor();1`);await sleep(650);
  ...
  await send('DOM.setFileInputFiles',{files:[IMG+'/p7.jpg',IMG+'/p13.jpg',IMG+'/p18.jpg'],objectId:oid});await sleep(1500);
  await ev(`el("toPresent").click()`);await sleep(700);
